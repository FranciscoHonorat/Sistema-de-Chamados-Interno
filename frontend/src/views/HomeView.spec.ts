...
// The shortcuts on the page itself; the side menu repeats them and is tested
// in AppNav.spec.ts.
function menuLinks(): Array<[string, string | null]> {
  return within(screen.getByRole('navigation', { name: 'Atalhos' }))
    .getAllByRole('link')
...
describe('HomeView', () => {
  ...
  it('summarizes the visible tickets and links the most recent ones', async () => {
    const api = fakeTicketsApi({
      list: vi.fn().mockResolvedValue([
        { ticket_id: 't-1', title: 'Impressora', description: 'x', status: 'Open', created_at: '2026-09-20T13:00:00Z' },
        { ticket_id: 't-2', title: 'Cadeira', description: 'x', status: 'Open', created_at: '2026-09-22T13:00:00Z' },
        { ticket_id: 't-3', title: 'Monitor', description: 'x', status: 'Closed', created_at: '2026-09-21T13:00:00Z' },
      ]),
    })
    renderHomeFor({ id: 'admin-1', name: 'Administradora', role: 'admin' }, fakeAuthService(), api)

    const recent = within(await screen.findByRole('region', { name: 'Chamados recentes' }))
    expect((await recent.findAllByRole('link', { name: /Impressora|Cadeira|Monitor/ })).map((link) => link.textContent?.trim())).toEqual([
      'Cadeira',
      'Monitor',
      'Impressora',
    ])
    const summary = within(screen.getByRole('region', { name: 'Resumo' }))
    expect(summary.getByText('Abertos').closest('div')).toHaveTextContent('2')
    expect(summary.getByText('Fechados').closest('div')).toHaveTextContent('1')
  })
