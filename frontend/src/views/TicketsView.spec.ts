...
describe('TicketsView', () => {
  ...
  it('narrows the list down to the titles matching the search', async () => {
    await renderTicketsAt(paths.tickets, vi.fn().mockResolvedValue(tickets))
    await screen.findByText('Impressora')

    await userEvent.type(screen.getByRole('searchbox', { name: 'Buscar chamados pelo título' }), 'cad')

    expect(rowTexts().map((row) => row[0])).toEqual(['Cadeira'])
  })

  it('says why the list is empty when nothing matches the search', async () => {
    await renderTicketsAt(paths.tickets, vi.fn().mockResolvedValue(tickets))
    await screen.findByText('Impressora')

    await userEvent.type(screen.getByRole('searchbox', { name: 'Buscar chamados pelo título' }), 'teclado')

    expect(screen.getByText('Nenhum chamado encontrado')).toBeInTheDocument()
    expect(screen.getByText('Nenhum título combina com a busca.')).toBeInTheDocument()
  })

  it('explains when the list cannot be loaded and tries again on request', async () => {
    const list = vi.fn().mockRejectedValueOnce(new Error('offline')).mockResolvedValueOnce(tickets)
    await renderTicketsAt(paths.tickets, list)

    expect(await screen.findByRole('alert')).toHaveTextContent('Não foi possível carregar os chamados')
