...
func newTestRouter() *gin.Engine {
	...
	handler := httpapi.NewHandler(httpapi.UseCases{
		...
		OpenTicket:             command.NewOpenTicketUseCase(store, c, responsibles),
		...
		AssignTicket:           command.NewAssignTicketUseCase(store, c, responsibles),
...
func TestTicketLifecycle(t *testing.T) {
	...
	w = doRequest(t, router, http.MethodPost, "/tickets/"+opened.TicketID+"/assign", map[string]string{"assignee_id": "user-1"})
