...
func (s *EventStore) LoadMany(ctx context.Context, aggregateIDs []uuid.UUID) (map[uuid.UUID][]event.Event, error) {
	...
	rows, err := s.pool.Query(ctx,
		`SELECT aggregate_id, event_type, payload, occurred_at FROM ticket_events
		 WHERE aggregate_id = ANY($1) ORDER BY aggregate_id, version ASC`,
		aggregateIDs,
	)
	if err != nil {
		return nil, err
	...
	for rows.Next() {
		...
		var payload []byte
		var occurredAt time.Time
		if err := rows.Scan(&aggregateID, &eventType, &payload, &occurredAt); err != nil {
			return nil, err
		}
		...
		histories[aggregateID] = append(histories[aggregateID], e)
	}
