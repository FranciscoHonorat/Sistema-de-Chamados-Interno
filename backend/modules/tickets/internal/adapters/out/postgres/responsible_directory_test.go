...
func TestResponsibleDirectory(t *testing.T) {
	...
	t.Run("should tell whether a responsible is registered", func(t *testing.T) {
		id := newTestResponsibleID(t, pool)
		require.NoError(t, directory.Upsert(context.Background(), id, "Registered"))

		registered, err := directory.Exists(context.Background(), id)
		require.NoError(t, err)
		unknown, err := directory.Exists(context.Background(), id+"-unknown")
		require.NoError(t, err)
