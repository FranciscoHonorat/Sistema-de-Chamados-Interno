...
func (d *ResponsibleDirectory) Exists(ctx context.Context, id string) (bool, error) {
	var exists bool
	err := d.pool.QueryRow(ctx, `SELECT EXISTS (SELECT 1 FROM responsibles WHERE id = $1)`, id).Scan(&exists)
	return exists, err
}
