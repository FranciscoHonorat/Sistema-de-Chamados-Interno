...
func ticketHandledBy(t *testing.T, store *outtest.EventStore, agentID, stage string) {
	...
	require.NoError(t, command.NewAssignTicketUseCase(store, cache, outtest.Agents()).Execute(ctx, command.AssignTicketInput{Actor: testAdmin, TicketID: id, AssigneeID: agentID}))
