...
func (s *EventStore) LoadMany(_ context.Context, aggregateIDs []uuid.UUID) (map[uuid.UUID][]event.Event, error) {
	s.LoadManyCalls++
	histories := make(map[uuid.UUID][]event.Event, len(aggregateIDs))
	for _, id := range aggregateIDs {
		if events, ok := s.streams[id]; ok {
			histories[id] = events
		}
	}
...
func (d *ResponsibleDirectory) Exists(_ context.Context, id string) (bool, error) {
	if d.Err != nil {
		return false, d.Err
	}
	return slices.Contains(d.Responsibles, id), nil
}

func Agents() *ResponsibleDirectory {
	return &ResponsibleDirectory{
...
func (s *NotificationStore) Add(_ context.Context, notifications []notification.Notification) error {
	...
	for _, n := range notifications {
		if !slices.ContainsFunc(s.Added, func(added notification.Notification) bool { return added.ID == n.ID }) {
