...
func TestNotifyAccountRequestUseCase(t *testing.T) {
	...
	t.Run("notifies only once when the same event is delivered again", func(t *testing.T) {
		store := &outtest.NotificationStore{}
		uc := command.NewNotifyAccountRequestUseCase(store, now)
		input := command.AccountRequestInput{EventID: "event-1", Kind: command.AccountSignUp, EmployeeID: "user-9", Name: "Maria Lima"}
	...
	t.Run("notifies once per event", func(t *testing.T) {
		store := &outtest.NotificationStore{}
		uc := command.NewNotifyAccountRequestUseCase(store, now)

		require.NoError(t, uc.Execute(context.Background(), command.AccountRequestInput{EventID: "event-1", Kind: command.AccountPasswordReset, EmployeeID: "user-1", Name: "Usuário Padrão"}))
		require.NoError(t, uc.Execute(context.Background(), command.AccountRequestInput{EventID: "event-2", Kind: command.AccountPasswordReset, EmployeeID: "user-1", Name: "Usuário Padrão"}))
