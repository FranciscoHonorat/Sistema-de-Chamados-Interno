...
func TestAuditTrail(t *testing.T) {
	t.Run("should record who performed each action on the ticket", func(t *testing.T) {
		...
		opened, err := command.NewOpenTicketUseCase(store, cache, outtest.Agents()).Execute(context.Background(), command.OpenTicketInput{
			Actor: testUser, Title: "Printer", Description: "Broken",
		})
		require.NoError(t, err)
		require.NoError(t, command.NewAssignTicketUseCase(store, cache, outtest.Agents()).Execute(context.Background(), command.AssignTicketInput{
