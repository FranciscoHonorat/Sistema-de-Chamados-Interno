...
func ticketAssignedToAgent(t *testing.T) (*outtest.EventStore, string) {
	...
	require.NoError(t, command.NewAssignTicketUseCase(store, newTestCache(), outtest.Agents()).Execute(context.Background(),
...
func TestCommandPermissions(t *testing.T) {
	...
	assign := func(store *outtest.EventStore, by actor.Actor, id string) error {
		return command.NewAssignTicketUseCase(store, newTestCache(), outtest.Agents()).Execute(context.Background(),
