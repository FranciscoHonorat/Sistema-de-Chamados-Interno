...
func TestChangePasswordEndpoint(t *testing.T) {
	...
	t.Run("lets someone with a temporary password replace it", func(t *testing.T) {
		router := newTestRouter()

		w := doAuthorizedRequest(router, http.MethodPost, "/auth/change-password", "pending-agent-token", `{"current_password":"senha123","new_password":"outra-senha"}`)
		require.Equal(t, http.StatusNoContent, w.Code)

		login := doRequest(router, http.MethodPost, "/auth/login", `{"username":"bruno","password":"outra-senha"}`)
		assert.Equal(t, http.StatusOK, login.Code)
	})
