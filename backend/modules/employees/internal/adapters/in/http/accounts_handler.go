...
func (h *Handler) ChangePassword(c *gin.Context) {
	...
	output, err := h.useCases.ChangePassword.Execute(c.Request.Context(), CallerFrom(c), application.ChangePasswordInput{
		Current: body.CurrentPassword,
		New:     body.NewPassword,
	})
	if err == nil {
		h.setRefreshCookie(c, output.RefreshToken, output.RefreshExpiresIn)
	}
	respondStatus(c, http.StatusNoContent, err)
}
