...
func newTestRouter() *gin.Engine {
	...
	verifier := outtest.TokenVerifier{Callers: map[string]out.Caller{
		...
		"pending-agent-token": {ID: "agent-2", Role: employee.RoleSupport, MustChangePassword: true},
		"pending-admin-token": {ID: "admin-1", Role: employee.RoleAdmin, MustChangePassword: true},
	}}
...
func TestListEmployeesEndpoint(t *testing.T) {
	...
	t.Run("is forbidden until a temporary password is replaced", func(t *testing.T) {
		w := get("Bearer pending-admin-token")
