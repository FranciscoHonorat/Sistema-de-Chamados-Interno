...
func (s *Sessions) Consume(ctx context.Context, refreshToken string) (string, error) {
	...
	now := s.now()
	rotated, err := s.store.Rotate(ctx, hash, now)
	if err != nil {
		return "", err
	}
	if rotated {
	...
	current, found, err := s.store.FindByHash(ctx, hash)
	if err != nil {
		return "", err
	}
	if found && current.RotatedWithin(now, RefreshReuseWindow) {
		return current.EmployeeID(), nil
	}
