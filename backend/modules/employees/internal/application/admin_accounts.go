...
func (uc *IssueTemporaryPasswordUseCase) Execute(ctx context.Context, caller out.Caller, id string) (string, error) {
	...
	// End the sessions before replacing the password: if this fails nothing
	// changed, whereas failing after the save would lose the only copy of the
	// new password and lock the employee out.
	if err := uc.sessions.EndAll(ctx, e.GetID()); err != nil {
		return "", err
	}
...
func NewChangePasswordUseCase(repo out.EmployeeRepository, hasher out.PasswordHasher, sessions *Sessions) *ChangePasswordUseCase {
	return &ChangePasswordUseCase{repo: repo, hasher: hasher, sessions: sessions}
}

// Execute replaces the caller's password, ends every session opened with the
// old one (another device, or whoever learned it) and starts a fresh session
// for the caller.
func (uc *ChangePasswordUseCase) Execute(ctx context.Context, caller out.Caller, input ChangePasswordInput) (SessionOutput, error) {
