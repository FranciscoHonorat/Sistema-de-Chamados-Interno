...
func TestChangePasswordUseCase(t *testing.T) {
	t.Run("replaces the password after checking the current one", func(t *testing.T) {
		...
		_, err := application.NewChangePasswordUseCase(repo, &outtest.PasswordHasher{}, newTestSessions(outtest.NewRefreshTokenStore())).Execute(context.Background(), adminCaller,
	...
	t.Run("ends the sessions opened with the old password and starts a new one", func(t *testing.T) {
		repo, sessions, store, oldRefreshToken := loggedInAdmin(t)

		output, err := application.NewChangePasswordUseCase(repo, &outtest.PasswordHasher{}, sessions).Execute(context.Background(), adminCaller,
			application.ChangePasswordInput{Current: "secret", New: "nova-senha-forte"})

		require.NoError(t, err)
		require.NotEmpty(t, output.RefreshToken)
		for hash, token := range store.Tokens {
			assert.Equal(t, hash != "sha:"+output.RefreshToken, token.IsRevoked(), hash)
		}
		_, err = sessions.Consume(context.Background(), oldRefreshToken)
		assert.ErrorIs(t, err, domainErr.ErrInvalidRefreshToken)
	})

	t.Run("refuses a wrong current password", func(t *testing.T) {
		_, err := application.NewChangePasswordUseCase(repoWithAdmin(), &outtest.PasswordHasher{}, newTestSessions(outtest.NewRefreshTokenStore())).Execute(context.Background(), adminCaller,
